...
export function nativeSddpGodPrompt(godRoles: AgentRole[]): string {
  ...
  return [
    ...
    SDDP_LIFECYCLE,
    '- EPIC CARD per feature: when a feature starts, create its epic card with `hive_add_task { title:"Feature <feature>", feature:"<feature>", description:"<the FEATURE REQUEST — what to build>", epic:true }` and ASSIGN it to a spawned planner desk. Put the feature request in the `description` (the host engine hands it to spec-author). The epic carries the milestone checklist (Specify→…→QC) that advances (`hive_update_task {advanceMilestone}`) as each artifact lands. Create exactly ONE epic per feature (the implement cards below are NOT epics).',
