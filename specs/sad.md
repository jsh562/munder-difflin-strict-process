...
- **Dual per-agent rendering paths (event-source-gated)**: The renderer presents one per-agent panel with two rendering paths selected by event source — authentic PTY bytes for Claude (xterm pool, untouched), and a synthesized terminal transcript (React/DOM) built directly from the normalized `AgentEvent` stream for native agents, plus an optional structured turns/tool-calls/token-usage tab available for native AND Claude desks; capability-degradation/api-error notices surface inline in both, and the operator can input/steer a native desk through the existing `ProviderRuntime.send` seam. Main forwards the in-process `AgentEvent` bus to the renderer over a per-agent IPC channel; a durable single-writer per-agent append-only event log (JSONL, mirroring `cost-ledger.jsonl`) persists the run so the transcript/structured view replay after an app restart — carrying only `AgentEvent` fields, never secrets (ADR-0007). The view-models add no `AgentEvent`/`AgentUsageSample`/`ToolSpan` shape change; token usage is display passthrough (cost authority stays at the usage seam); the transcript is virtualized (full run retained, no eviction). Realizes ADR-0010; the event-forward IPC + durable per-agent event log are reusable seams for any epic touching the per-agent view (e.g., live provider switch).
