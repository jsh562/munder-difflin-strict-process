...
- **Per-agent panel parity (native rendering)**: Every desk's per-agent panel stays a terminal regardless of provider — a native (non-Claude) desk renders as a synthesized terminal-style transcript of its activity (streamed text, tool calls/results, thinking) with an optional structured tab (turns, tool calls, token usage) and inline capability-degradation/error notices, while Claude desks keep their authentic terminal view. Operator observability is uniform across providers at the panel level.
